...
const selectedRow = ref<AssignmentRow | null>(null)
...
const { data, status, refresh } = await useFetch<{ success: boolean, data: AssignmentRow[] }>('/api/billing/assignments', {
...
async function onSubmit(event: FormSubmitEvent<Schema>) {
  await $fetch(editingAssignmentId.value ? `/api/billing/assignments/${editingAssignmentId.value}` : '/api/billing/assignments', {
    method: editingAssignmentId.value ? 'PATCH' : 'POST',
    body: event.data
  })
  toast.add({ title: 'Przypisanie zapisane', color: 'success' })
  open.value = false
  resetForm()
  await refresh()
}
...
async function deleteAssignment(row: AssignmentRow) {
  if (!window.confirm(`Usunąć przypisanie ${row.customer.fullName} / ${row.tariff.name}?`)) return
  await $fetch(`/api/billing/assignments/${row.id}`, { method: 'DELETE' })
  toast.add({ title: 'Przypisanie usunięte', color: 'success' })
  await refresh()
}

function showDetails(row: AssignmentRow) {
  selectedRow.value = row
  detailsOpen.value = true
}

function rowContextItems(row: AssignmentRow): ContextMenuItem[][] {
  return [[
    { label: 'Edytuj przypisanie', icon: 'i-lucide-pencil', onSelect: () => openEdit(row) },
    { label: 'Szczegóły przypisania', icon: 'i-lucide-panel-right-open', onSelect: () => showDetails(row) },
    { label: 'Usuń przypisanie', icon: 'i-lucide-trash-2', color: 'error', onSelect: () => deleteAssignment(row) },
...
<template>
  <UDashboardPanel id="billing-assignments" :ui="{ body: 'p-0 sm:p-0 gap-0 sm:gap-0' }">
    <template #header>
      <UDashboardNavbar title="Przypisania">
        <template #leading>
        ...
        <template #right>
          ...
          <USelect
            ...
            class="w-44"
          />
          <USlideover v-model:open="open" :title="editingAssignmentId ? 'Edytuj przypisanie' : 'Dodaj przypisanie'">
            <UButton label="Dodaj przypisanie" icon="i-lucide-badge-dollar-sign" @click="openCreate" />
            <template #body>
              <UForm
                :schema="schema"
                :state="state"
